...
async function applyReleasePlan(releasePlan, packages, config = defaultConfig, snapshot) {
  ...
  let releasesWithPackage = releases.map(release => {
    ...
    return _objectSpread(_objectSpread({}, release), pkg);
...
async function getNewChangelogEntry(releasesWithPackage, changesets, config, cwd) {
  if (!config.changelog) {
    return Promise.resolve(releasesWithPackage.map(release => _objectSpread(_objectSpread({}, release), {}, {
      changelog: null
    })));
  }
  ...
  let moddedChangesets = changesets.map((cs, i) => _objectSpread(_objectSpread({}, cs), {}, {
    commit: commits[i]
  }));
